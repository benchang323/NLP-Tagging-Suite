...
class HiddenMarkovModel(nn.Module):
    ...
    def log_forward(self, sentence: Sentence, corpus: TaggedCorpus) -> TorchScalar:
        ...
        # k = 4 (C, H, EOS, BOS)
        # n = 12 (BOS, ..., sentence, ..., EOS)
        # V = 3 (1, 2, 3)

        # Alpha shape = (n, k) = (12, 4)
        # A (transition) shape = (k, k) = (4, 4)
        # B (emission) shape = (k, V) = (4, 3)
        
        n = len(sent) - 2 # len(sent)=sentence length + 2(BOS,EOS); Initialize n=sentence length

        # Initialize BOS of alpha
        ...
        for j in range(1,n+1):  
            ...
            alpha[j] = alpha[j-1] * (1/ scales[j-1]) @ self.A * self.B[:, obv] 

            # Efficient tag handling
            if tag is None: sup = torch.ones(self.k)
            else:
            ...
            if alpha[j].min() < 1e-20:
                scales[j-1] = alpha[j].max()
                alpha[j] = alpha[j] * (1 / scales[j-1])
        
        scales[n] = torch.max(alpha[n]).item()
        # Set alpha_EOS(n+1)=prob(obv_1, obv_2, ..., obv_n)
        alpha[n+1][self.eos_t] = alpha[n] * (1 / scales[n]) @ self.A[:, self.eos_t]
        # print(alpha[n+1][self.eos_t].log() + torch.sum(scales.log()))

        return alpha[n+1][self.eos_t].log() + scales.log().sum()
    ...
    def viterbi_tagging(self, sentence: Sentence, corpus: TaggedCorpus) -> Sentence:
        ...
        backpoints = []
        scales = torch.ones(len(sent) - 2)  # Initialize scales
        ...
        for j in range(1, len(sent) - 1):
            ...
            # Scale to avoid underflow
            scales[j-1] = torch.max(alpha[j-1]).item()
            max_value = max_value * self.B[:, word] * (1 / scales[j-1])
    ...
    def train(self,
        ...
        # optimizer = optim.Adam(self.parameters(), lr=lr, )  # optimizer knows what the params are
