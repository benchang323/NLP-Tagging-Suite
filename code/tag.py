...
from crf import ConditionalRandomField as CRFModel
...
def parse_args() -> argparse.Namespace:
    ...
    if not args.model and not args.lexicon:
        print("No lexicon file provided, using one hot encoding...")
...
def main() -> None:
    ...
    if args.model is not None:
        if args.crf: model = CRFModel.load(Path(args.model), gpu=args.gpu)
        else: model = HiddenMarkovModel.load(Path(args.model), gpu=args.gpu)
        ...
        if args.lexicon is not None: lexicon = build_lexicon(train, embeddings_file=Path(args.lexicon), log_counts=args.awesome, affixes=args.awesome, shape=args.awesome)
        else: lexicon = build_lexicon(train, one_hot=True, log_counts=args.awesome, affixes=args.awesome, shape=args.awesome)
        
        if args.crf: model = CRFModel(tagset, vocab, lexicon, unigram=args.unigram)
        else: model = HiddenMarkovModel(tagset, vocab, lexicon, unigram=args.unigram)
