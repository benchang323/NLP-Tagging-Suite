...
from crf import ConditionalRandomFieldBiRNN
...
# Initialize an HMM
lexicon = build_lexicon(entrain, embeddings_file=Path('words-50.txt'), log_counts=False, affixes=False)  # works better with more attributes!

# hmm = HiddenMarkovModel(entrain.tagset, entrain.vocab, lexicon)
hmm = ConditionalRandomFieldBiRNN(entrain.tagset, entrain.vocab, lexicon)
...
hmm.train(corpus=ensup, loss=loss_sup, minibatch_size=30, evalbatch_size=1000, lr=0.01, reg=1, tolerance=.0001) 
...
hmm.train(corpus=entrain, loss=loss_dev, minibatch_size=30, evalbatch_size=10000, lr=0.0001, reg=0, tolerance=.0001)
